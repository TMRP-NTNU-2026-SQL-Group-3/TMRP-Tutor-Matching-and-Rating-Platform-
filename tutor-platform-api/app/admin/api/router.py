...
from app.shared.infrastructure.base_repository import BaseRepository, coerce_csv_value, quote_columns, validate_columns
...
def _write_csv(file_path: str, rows: list[dict], fieldnames: list[str] | None = None) -> None:
    if not rows:
        return
...
def list_users(user=Depends(require_role("admin")), conn=Depends(get_db)):
    ...
    rows = repo.fetch_all(
        "SELECT user_id, username, role, display_name, phone, email, created_at FROM users ORDER BY user_id"
    )
...
def import_csv(
    ...
    try:
        ...
        conn.rollback()
        raise
    return ApiResponse(success=True, data={"count": len(rows)}, message=f"已匯入 {len(rows)} 筆資料至 {table_name}")


@router.get("/export/{table_name}", summary="匯出 CSV")
def export_csv(table_name: str, user=Depends(require_role("admin")), conn=Depends(get_db)):
    _validate_table(table_name)
    ...
    _write_csv(str(export_path), rows)
    safe_filename = re.sub(r'[^A-Za-z0-9_\-]', '', table_name) + ".csv"
    return FileResponse(path=str(export_path), filename=safe_filename, media_type="text/csv")


@router.post("/reset", summary="清空資料庫", response_model=ApiResponse)
def reset_database(confirm: bool = Query(...), user=Depends(require_role("admin")), conn=Depends(get_db)):
    if not confirm:
...
def system_status(user=Depends(require_role("admin")), conn=Depends(get_db)):
    ...
    role_counts = repo.fetch_all("SELECT role, COUNT(*) AS cnt FROM users GROUP BY role")
    roles = {r["role"]: r["cnt"] for r in role_counts}
    status_counts = repo.fetch_all("SELECT status, COUNT(*) AS cnt FROM matches GROUP BY status")
    match_statuses = {r["status"]: r["cnt"] for r in status_counts}
    return ApiResponse(success=True, data={"table_counts": counts, "role_counts": roles, "match_statuses": match_statuses}, message="系統狀態查詢完成")
...
def get_task_status(task_id: str, user=Depends(require_role("admin"))):
    ...
    try:
        ...
        # Bug #14: 不靜默吞噬；記錄錯誤後降級為「pending」以避免管理面板崩潰
    ...
    try:
        ...
        try:
            ...
            # Bug #14: 收斂例外類型至 pickle 反序列化會拋出的具體類型
...
def import_all(
    ...
    try:
        ...
        raise DomainException("上傳的檔案不是有效的 ZIP 壓縮檔")
    csv_files = {}
