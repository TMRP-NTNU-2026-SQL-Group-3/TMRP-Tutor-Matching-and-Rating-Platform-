...
# M-06: progressive lockout — a 1-hour bucket layered on top of the 5/15min
# bucket. The short bucket still gates each 15-min burst to 5 real guesses,
# but the long bucket caps sustained throughput: after 15 total attempts
# (including rejected ones that still record a hit), the account is locked
# for the remainder of the 1-hour window. Effective cap: ~360 guesses/24h
# vs the previous ~480. The 6-hour escalation that briefly shipped was
# pulled back — an anonymous attacker triggering it could lock any known
# account out for a full quarter of the day, which made the lockout itself
# more disruptive than the brute-force scenario it was meant to defend
# against. 1 hour preserves the same brute-force ceiling per cycle while
# giving legitimate users a reasonable self-recovery window.
