...
from app.admin.domain.tables import ALLOWED_TABLES, EXPORTABLE_TABLES
...
from app.admin.infrastructure.csv_io import write_csv
...
def import_csv_task(table_name: str, csv_content: str, admin_user_id: int, request_id: str | None = None) -> dict:
    ...
    try:
        ...
        logger.info(
            "Imported %d/%d rows into %s (%d errors)", imported, len(rows), table_name, len(errors),
            extra=_task_extra(request_id, table=table_name, rows=imported),
        )
        result = {"table": table_name, "count": imported}
        if errors:
