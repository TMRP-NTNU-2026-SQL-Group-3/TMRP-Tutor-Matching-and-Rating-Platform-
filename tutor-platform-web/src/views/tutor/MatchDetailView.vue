  ...
  <div>
    ...
    <div v-else-if="match">
      ...
      <div class="flex flex-wrap gap-2 mb-6">
        <button v-if="match.status === 'pending'" @click="doAction('accept')"
          class="bg-green-600 hover:bg-green-700 text-white rounded-lg px-4 py-2 text-sm font-medium transition-colors">
          接受邀請
        </button>
        <button v-if="match.status === 'pending'" @click="doAction('reject')"
...
const reviewLabels = computed(() => {
  if (reviewForm.review_type === 'tutor_to_parent')
    return ['配合度', '溝通態度', '準時付費', '整體滿意度']
  return ['學習態度', '完成作業', '課堂表現', '整體進步']
})
